...
int connect_to_server(char *hostname, int portno) {
    ...
    // Copy the first address from h_addr_list to sin_addr.s_addr
    if (server->h_addr_list[0] != NULL) {
        memcpy(&serv_addr.sin_addr.s_addr, server->h_addr_list[0], server->h_length);
    } else {
        fprintf(stderr, "ERROR, no address found for the host\n");
        exit(0);
    }
...
void take_turn(int sockfd) {
    ...
    while (1) {
        printf("Enter 0-8 to make a move, or 9 for the number of active players: ");
